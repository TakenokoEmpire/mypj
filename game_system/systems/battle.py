    ...
    def vlookup_plus(self, sheet, index, column_order,left_index_position):
        ...
        for i in range(500):
            ...
            if str(sheet.cell(row=i + 1, column=left_index_position).value) == index:
                output = sheet.cell(row=i+1, column=left_index_position+column_order-1).value
                return output
        return False
...
class Battle(Function):
    ...
    def __init__(self, dungeon_type, dungeon_num):
        ...
        self.equip_sheetsoubi_name_columnnum = self.hindex(self.book["装備"],"name")
    ...
    def equip_checker(self):
        ...
        for i in range(self.equip_qty):
            equip_name = self.vhlookup(self.mysheet, self.equip_position[i], 1,"name",self.equip_index_rownum) 
            for j in range(len(update_stat)):
                self.mysheet[self.xy_index(self.mysheet, self.equip_position[i], 1, update_stat[j], self.equip_index_rownum, "excel")] = self.vhlookup(self.book["装備"],equip_name,self.equip_sheetsoubi_name_columnnum,update_stat[j],1)
        # 装備欄に記録されたhp,atkの補正値をステータスに反映する(補正値の合計sum_statを計算し、該当する位置に貼り付ける)
        for j in range(len(update_stat)):
            for i in range(self.equip_qty):
                sum_stat[j] += self.vhlookup(self.mysheet, self.equip_position[i], 1,update_stat[j],self.equip_index_rownum)
            self.mysheet[self.xy_index(self.mysheet, update_stat[j], 1, "equip", 1, "excel")] = sum_stat[j]
