...
groupinfo = []
history = []
wholeinfo = {"phase": 0, "turn": 1, "confirmed_blow_and_hit": 0, "confirmed_hit": 0, "noob": 0, "rough_blow": 0, "special_mode": 0}
blow_list = []
hit_list = [-1, -1, -1, -1, -1]
gnum_count = -1
ram = [] #一時保存用。ターン終了時に削除されるので注意
phase_list = {0: "Early phase", 1: "Early phase", 2: "Early phase", 3: "Middle phase", 4: "FINAL phase"}
game_record = []
...
def zero_checker(x):
    if x==0:
        ...
        return 1
def curse(x):
    if x == 3 and wholeinfo["special_mode"] >= 1:
        return 1
    else:
        return 100
...
def fugaku(ninfo, ginfo, hist, winfo):
    ...
    if winfo["phase"] < 3:
        ...
        priority_value = [(1000 - 100 * j["leng"] - (j["blow"]+j["hit"]))* zero_checker((j["blow"]+j["hit"])*(j["leng"]-(j["blow"]+j["hit"])))*curse(j["gnum"]) for j in ginfo]
        ...
        if target_length == 5:
            ...
            if target_gnum == 0:
                guess_mid_str = "f0123"
                ram.append({"turn":winfo["turn"], "group_member":[0,1,2,3], "non_group_member":[4], "group_qty":1, "gnum0":0, "gnum1":3, "gnum2":-1, "guess_mid_list":[15,0,1,2,3]})
                for i in [0,1,2,3]:
                    ninfo[i]["gnum"] = 3
            elif target_gnum == 1:
                guess_mid_str = "f5678"
                ram.append({"turn": winfo["turn"], "group_member": [5, 6, 7, 8], "non_group_member": [9], "group_qty": 1, "gnum0": 1, "gnum1": 3, "gnum2": -1, "guess_mid_list": [15, 5, 6, 7, 8]})
                for i in [5,6,7,8]:
                    ninfo[i]["gnum"] = 3
            else:
                guess_mid_str = "fabcd"
                ram.append({"turn":winfo["turn"], "group_member":[10,11,12,13], "non_group_member":[14], "group_qty":1, "gnum0":2, "gnum1":3, "gnum2":-1, "guess_mid_list":[15,10,11,12,13]})
        ...
        ram.append({"mode":"normal", "turn":winfo["turn"], "group_member":member, "non_group_member":bocchi, "group_qty":len(list(filter(pos,[gnum1,gnum2]))), "gnum0":target_gnum, "gnum1":gnum1, "gnum2":gnum2, "guess_mid_list":guess_mid_list})
        return (guess_mid_str)
...
d2run.post_hidden(ans=define_answer("")) #指定する場合はdefine_answer()のカッコ内に入力。ランダムでよい場合は""と入力
drun.get_table()
#368af
def ending():
    global ans_str
...
while True:
    ...
    print()
    print("--------------------------------------------")
    print("You are in "+phase_list[wholeinfo["phase"]])
