...
class ShowGame:
    ...
    def history_show(self):
        ...
        if self.turn > 10:
            for i in range(10):
                for j in range(5):
                    self.screen.blit(self.guess_list[i][j], (30+j*40,55+i*40,30,30))
                turn = font.render("{}".format(i+1), True, (255,255,255))
                self.screen.blit(turn, (3,60+i*40))
                pygame.draw.line(self.screen,(100,100,100),(25,50+i*40),(25,90+i*40),1)
                pygame.draw.line(self.screen,(100,100,100),(0,90+i*40),(360,90+i*40),1)
    ...
    def history2_show(self):
        ...
        if self.turn > self.history_count*10:
            for i in range((self.history_count-1)*10,self.history_count*10):
                for j in range(5):
                    self.screen.blit(self.guess_list[i][j], (30+j*40,55+(i-(self.history_count-1)*10)*40,30,30))
                turn = font.render("{}".format(i+1), True, (255,255,255))
                self.screen.blit(turn, (3,60+(i-(self.history_count-1)*10)*40))
                pygame.draw.line(self.screen,(100,100,100),(25,50+(i-(self.history_count-1)*10)*40),(25,90+(i-(self.history_count-1)*10)*40),1)
                pygame.draw.line(self.screen,(100,100,100),(0,90+(i-(self.history_count-1)*10)*40),(360,90+(i-(self.history_count-1)*10)*40),1)
                hit_blow = font.render("Hit:{} Blow:{}".format(self.hit_list[i], self.blow_list[i]), True, (255,255,255))
                self.screen.blit(hit_blow, (230,60+(i-(self.history_count-1)*10)*40))
            ...
            if (event.type == pygame.MOUSEBUTTONUP) and (event.button == 1):
                ...
                if self.enter_buttonrect.collidepoint(event.pos):
                    ...
                    if self.error_count == 0:
                        guess_mark = [self.marks_s[self.num[0]],self.marks_s[self.num[1]],self.marks_s[self.num[2]],self.marks_s[self.num[3]],self.marks_s[self.num[4]]]
                        self.guess_list.append(guess_mark) # マークの履歴
                        # self.guess = self.convert() # マークから文字列へ変換
                        #self.hit = 1
                        #self.blow = 2 #正誤の判定
                        self.hit, self.blow = numberguess.judge_guess(guess=self.num) #判定
    ...
    def convert(self):
        ...
        for n in range(5):
            ...
            if num_convert[n] == "15":
                num_convert[n] = "f"
        self.guess = num_convert[0]+num_convert[1]+num_convert[2]+num_convert[3]+num_convert[4]
    ...
    def clear(self, enemy_level:int=0):
        ...
        exp = font2.render("EXP:{}".format(self.exp_list[enemy_level]), True, (255,255,255))
