...
class ShowGame(battle.Battle, town.Town):
    ...
    def __init__(self,
        ...
        self.flag = []
        self.lv_g = self.lv
        self.screen_size = 1 #画面サイズを決定するための変数
        self.screen_count = 0
    ...
    def set_mark(self):
        ...
        for n in range(5):
            self.mark_buttonrect.append(Rect((10+n*70)*self.screen_size,425*self.screen_size,60*self.screen_size,25*self.screen_size))  # マークの位置
    ...
    def normal_stage(self):
        ...
        if self.max_hp > 300:
            pygame.draw.line(self.screen,(0,200,0),(30*self.screen_size,40*self.screen_size),(30*self.screen_size+self.hp*self.screen_size*300/self.max_hp,40*self.screen_size),10*self.screen_size) # HPの表示
        else:
            pygame.draw.line(self.screen,(0,200,0),(30*self.screen_size,40*self.screen_size),((30+self.hp)*self.screen_size,40*self.screen_size),10*self.screen_size) # HPの表示
        ...
        if damage != 0:
            if self.max_hp >300:
                pygame.draw.line(self.screen,(200,0,0),(30*self.screen_size+self.hp*self.screen_size*300/self.max_hp,40*self.screen_size),(30*self.screen_size+(self.hp+damage)*self.screen_size*300/self.max_hp,40*self.screen_size),10*self.screen_size)
            else:
    ...
    def choice_screen(self, title, choice: List[str], message: List[str], dict_name, delete_dict_list_when_return: List[str] = []):
        ...
        font2 = pygame.font.SysFont(
            "bizudminchomediumbizudpminchomediumtruetype", 18*self.screen_size)
        # titleを表示
        ...
        for i, name in enumerate(choice):  # 選択肢の数だけ描画
            ...
            self.screen.blit(print_choice, (30*self.screen_size, (93+47*i)*self.screen_size))
        self.screen.blit(self.return_button_img, self.return_buttonrect)
        # メッセージを表示
        for i, name in enumerate(message):
            print_message = font2.render(name, True, "WHITE")
            self.screen.blit(print_message, (11*self.screen_size, (430+22*i)*self.screen_size))
