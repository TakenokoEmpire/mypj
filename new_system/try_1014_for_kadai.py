...
# import send_recieve_for_kadai
from send_recieve_for_kadai import SendReceive
import random
import math
import copy
"""
...
class AutoPlay():
    ...
    def define_answer(self):
        ...
        print("chosen_answer:"+self.ans_str)  # あらかじめ答えを知りたいときのみ有効化する
    ...
    def run(self,room_id_meta):
        ...
        #             pass
        # else:
        drun = SendReceive(room_id=room_id_meta,player_name = "D")
        drun.enter_room()
        drun.get_room()
        dic = {}
        ans_count = 0
        ...
        while dic["state"] != 3 :
            ...
            if dic["state"] == 2:
                if ans_count == 0:
                    self.define_answer()
                    drun.post_hidden(ans=self.ans_str)
                    ans_count += 1
                elif dic["now_player"] == "D":
                    break
        # and dic["now_player"] != "D"
        # # 指定する場合はdefine_answer()のカッコ内に入力。ランダムでよい場合は""と入力
        # d2run.post_hidden(ans="729b1")
...
battle_qty = 100
for i in range(battle_qty):
    print("Autoplay is on progress...{}/{}".format(i,battle_qty))
    AutoPlay("off").run(room_id + i)