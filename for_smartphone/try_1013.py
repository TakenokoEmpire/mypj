...
class AutoPlay():
    ...
    #try_1011追加分。サーバーを介さずにautoplayができるようになった。
    def judge_guess(self, guess):
        h = 0
        b = 0
        for i in range(len(guess)):
            if guess[i] == self.ans_str[i]:
                h += 1  # hがヒットの数
            else:
                if guess[i] in self.ans_str:
                    b += 1  # bがブローの数
        dic = {'guess':guess, 'hit':h, 'blow': b}
        return dic
    ...
    def run(self):
        ...
        while True:
            result = self.judge_guess(self.fugaku(
            ...
            """クリア判定"""
            if self.history[-1]["hit"] == 5 or self.wholeinfo["turn"] >= 40:
                self.ending()
                """ここで終了"""
                return self.history
